defmodule Ace.HTTP2.Frame.Settings do
  ...
  @type t :: %__MODULE__{
          ack: boolean,
          header_table_size: nil | non_neg_integer,
          enable_push: nil | boolean,
          max_concurrent_streams: nil | non_neg_integer,
          initial_window_size: nil | non_neg_integer,
          max_frame_size: nil | non_neg_integer,
          max_header_list_size: nil | non_neg_integer
        }
  ...
  @spec new(Enumerable.t() | Ace.HTTP2.Settings.t()) :: t()
  def new(parameters \\ []) do
    parameters = Enum.into(parameters, %{})
    %__MODULE__{ack: false} |> Map.merge(parameters)
  end

  @spec ack() :: __MODULE__.t()
  def ack() do
    %__MODULE__{ack: true}
  end

  @spec decode({4, binary, non_neg_integer, binary}) ::
          {:ok, t()} | {:error, :bad_settings_frame} | {:error, {:protocol_error, string}}
  ...
  def serialize(frame = %{ack: false}) do
    ...
    payload =
      Enum.reduce(@parameter_keys, [], fn key, acc ->
  ...
  defp parse_settings(binary, settings \\ %{})
  ...
  defp cast_setting(1, value) do
    {:ok, {:header_table_size, value}}
  end

  defp cast_setting(2, 0) do
    {:ok, {:enable_push, false}}
  end

  defp cast_setting(2, 1) do
    {:ok, {:enable_push, true}}
  end

  defp cast_setting(2, _) do
    {:error, {:protocol_error, "invalid value for enable_push setting"}}
  end

  defp cast_setting(3, value) do
    {:ok, {:max_concurrent_streams, value}}
  end

  # DEBT is minumum 0 or 1
  defp cast_setting(4, value) when 0 <= value and value <= 2_147_483_647 do
    {:ok, {:initial_window_size, value}}
  end

  defp cast_setting(4, _value) do
    {:error, {:protocol_error, "invalid value for initial_window_size setting"}}
  end

  defp cast_setting(5, value) when 16384 <= value and value <= 16_777_215 do
    {:ok, {:max_frame_size, value}}
  end

  defp cast_setting(5, _value) do
    {:error, {:protocol_error, "invalid value for max_frame_size setting"}}
  end

  defp cast_setting(6, value) do
    {:ok, {:max_header_list_size, value}}
  end

  defp cast_setting(identifier, value) when identifier > 6 do
    {:ok, {:unknown_setting, value}}
  end

  defp setting_parameter(:header_table_size, value) do
    <<1::16, value::32>>
  end

  defp setting_parameter(:enable_push, false) do
    <<2::16, 0::32>>
  end

  defp setting_parameter(:enable_push, true) do
    <<2::16, 1::32>>
  end

  defp setting_parameter(:max_concurrent_streams, value) do
    <<3::16, value::32>>
  end

  defp setting_parameter(:initial_window_size, value) do
    <<4::16, value::32>>
  end

  defp setting_parameter(:max_frame_size, value) do
    <<5::16, value::32>>
  end

  defp parameters_to_payload(parameters, payload \\ [])

  defp parameters_to_payload([], payload) do
    Enum.reverse(payload)
    |> :erlang.iolist_to_binary()
  end

  defp parameters_to_payload([{:header_table_size, value} | rest], payload) do
    payload = [<<1::16, value::32>> | payload]
    parameters_to_payload(rest, payload)
  end

  defp parameters_to_payload([{:max_frame_size, value} | rest], payload) do
...
defimpl Inspect, for: Ace.HTTP2.Frame.Settings do
  ...
  def inspect(settings = %{ack: false}, _opts) do
    setting_params =
      Enum.flat_map(@parameter_keys, fn key ->
